...
import {SelectItem, OverlayPanel, Button, ToggleButton, ContextMenu, MenuItem, LazyLoadEvent} from 'primeng/primeng';
    ...
    public genres:Genres, public tags:TagService) {
          ...
          this.tracks.getLibrary(this.selectedCurrencyId, this.trackListFilters, this.trackListFilters.currentPage).subscribe( (res) => {
            ...
            if(res.restrictions) {
              ...
              switch(res.restrictions) {
                ...
                this.trackListRestrictions = 'You do not have a current package on your account. To access your library, purchase a new package.';
                break;
      ...
      ngOnInit(){
        ...
        setCurrency(currency) {
          ...
          console.log('currency changed on track list', currency);
          ...
          this.editors.getCurrencyEditors(currency._id).subscribe( (res) => {
            ...
            this.currencyEditors = res.CurrencyEditors.map( (editor) => {
              return {label: editor.name, value: editor._id }
            });
            this.currencyEditors.forEach((editor) => {
              this.currencyEditorsById[editor.value] = editor.label;
            })
            console.log('this.currencyEditorsById', this.currencyEditorsById);
        ...
        adjustTableHeight() {
          ...
          if(this.trackTable && this.storeControls) {
            ...
            if(tableHeader && tableHeader.clientHeight) {
              ...
              if(this.showFilters) {
                var tableBodyHeight = contentAreaHeight - storeControlsHeight - tableHeaderHeight - 300;
              } else {
                var tableBodyHeight = contentAreaHeight - storeControlsHeight - tableHeaderHeight - 50;
              }
        ...
        //         this.tracks.totalRecords = res.Tracks.total;
        //         this.tracks.list = this.tracks.list.concat(res.Tracks.results);
        //         console.log('list is upldated HERE');
        ...
        console.log('filterTrackList', $event);
          $event = $event || {};
          ...
          if($event && $event.rows) {
            ...
            if($event.first != 0) {
              console.log('calculating', $event.first, this.trackListFilters.rows);
              this.trackListFilters.currentPage = 1 + ($event.first / this.trackListFilters.rows);
          ...
          console.log('requesting page ' + this.trackListFilters.currentPage);
          ...
          if(this.selectedCurrencyId) {
            ...
            if(this.activeTab != 'download-queue' && this.activeTab != 'library') {
              ...
              this.tracks.getTracks(this.selectedCurrencyId, this.trackListFilters, this.trackListFilters.currentPage).subscribe( (res) => {
                ...
                this.tracks.totalRecords = res.Tracks.total;
                this.tracks.list = this.tracks.list.concat(res.Tracks.results);
                console.log(this.tracks.list);
                console.log('totalRecords', this.tracks.totalRecords);
                console.log('this.tracks.list.length', this.tracks.list.length);
                this.selectedCollectionId = null;
                this._state.notifyDataChanged('spinner.hide', {});
                this._state.notifyDataChanged('spinner.show', {});
          ...
          return this.downloadTrackSetup(track).subscribe( (downloadResponse) => {
            ...
            if(downloadResponse.err) {
              ...
              if(autoDownload) {
                ...
                let body = JSON.stringify({ downloadId: downloadResponse.download });
                let headers = new Headers({ 'Content-Type': 'application/json' });
                let options = new RequestOptions({ headers: headers });

                this.http.post('/api/v1/members/download/get', body, options).map( (res) => res.json()).subscribe( (response) => {
      ...
      */
      ...
      createCrate(saveAsNewCopy?:boolean) {
        ...
        this.currentlyAppliedCrate = Object.assign(this.currentlyAppliedCrate, this.trackListFilters);
